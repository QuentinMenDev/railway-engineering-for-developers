import { useState, useCallback } from 'react';
import { getAllLessonIds, courseModules } from '../data/courseStructure';

const STORAGE_KEY = 'railway-course-progress-v2';
const allLessonIds = getAllLessonIds();

function loadInitial() {
  try {
    const saved = JSON.parse(localStorage.getItem(STORAGE_KEY));
    if (saved) return saved;
    // Migrate old format
    const old = JSON.parse(localStorage.getItem('railway-curriculum-progress') || '{}');
    if (old && Object.keys(old).length > 0) {
      const progress = { lessons: {}, quizzes: {} };
      Object.entries(courseModules).forEach(([modId, mod]) => {
        if (old[modId]) mod.lessons.forEach(l => { progress.lessons[l.id] = true; });
      });
      localStorage.setItem(STORAGE_KEY, JSON.stringify(progress));
      return progress;
    }
  } catch (e) { /* ignore */ }
  return { lessons: {}, quizzes: {} };
}

export function useProgress() {
  const [progress, setProgress] = useState(loadInitial);

  const toggleLesson = useCallback((id) => {
    setProgress(prev => {
      const next = { ...prev, lessons: { ...prev.lessons, [id]: !prev.lessons[id] } };
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(next)); } catch (e) { /* ignore */ }
      return next;
    });
  }, []);

  const saveQuiz = useCallback((lessonId, score, total) => {
    setProgress(prev => {
      const next = { ...prev, quizzes: { ...prev.quizzes, [lessonId]: { score, total } } };
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(next)); } catch (e) { /* ignore */ }
      return next;
    });
  }, []);

  const isModuleComplete = useCallback((modId) => {
    const mod = courseModules[modId];
    if (!mod) return false;
    return mod.lessons.every(l => progress.lessons[l.id]);
  }, [progress]);

  const lessonsDone = allLessonIds.filter(id => progress.lessons[id]).length;
  const totalItems = allLessonIds.length;
  const pct = totalItems > 0 ? Math.round((lessonsDone / totalItems) * 100) : 0;

  return { progress, toggleLesson, saveQuiz, isModuleComplete, pct, totalDone: lessonsDone, totalItems };
}
